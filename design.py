# -*- coding: utf-8 -*-

# Form implementation generated from reading ui file 'design.ui'
#
# Created by: PyQt5 UI code generator 5.15.6
#
# WARNING: Any manual changes made to this file will be lost when pyuic5 is
# run again.  Do not edit this file unless you know what you are doing.


from PyQt5 import QtCore, QtGui, QtWidgets


class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        MainWindow.setObjectName("MainWindow")
        MainWindow.resize(768, 159)
        MainWindow.setStyleSheet("")
        self.centralwidget = QtWidgets.QWidget(MainWindow)
        self.centralwidget.setObjectName("centralwidget")
        self.gridLayout = QtWidgets.QGridLayout(self.centralwidget)
        self.gridLayout.setObjectName("gridLayout")
        self.btnGenerator = QtWidgets.QPushButton(self.centralwidget)
        font = QtGui.QFont()
        font.setPointSize(15)
        self.btnGenerator.setFont(font)
        self.btnGenerator.setObjectName("btnGenerator")
        self.gridLayout.addWidget(self.btnGenerator, 1, 2, 1, 1)
        self.inputCpf = QtWidgets.QLineEdit(self.centralwidget)
        font = QtGui.QFont()
        font.setPointSize(15)
        self.inputCpf.setFont(font)
        self.inputCpf.setObjectName("inputCpf")
        self.gridLayout.addWidget(self.inputCpf, 0, 1, 1, 1)
        self.btnValidate = QtWidgets.QPushButton(self.centralwidget)
        self.btnValidate.setMinimumSize(QtCore.QSize(200, 0))
        font = QtGui.QFont()
        font.setPointSize(15)
        self.btnValidate.setFont(font)
        self.btnValidate.setObjectName("btnValidate")
        self.gridLayout.addWidget(self.btnValidate, 0, 2, 1, 1)
        self.label = QtWidgets.QLabel(self.centralwidget)
        font = QtGui.QFont()
        font.setPointSize(15)
        self.label.setFont(font)
        self.label.setAlignment(QtCore.Qt.AlignRight|QtCore.Qt.AlignTrailing|QtCore.Qt.AlignVCenter)
        self.label.setObjectName("label")
        self.gridLayout.addWidget(self.label, 1, 1, 1, 1)
        self.label_2 = QtWidgets.QLabel(self.centralwidget)
        font = QtGui.QFont()
        font.setPointSize(15)
        self.label_2.setFont(font)
        self.label_2.setObjectName("label_2")
        self.gridLayout.addWidget(self.label_2, 0, 0, 1, 1)
        self.labelCpf = QtWidgets.QLabel(self.centralwidget)
        font = QtGui.QFont()
        font.setPointSize(-1)
        self.labelCpf.setFont(font)
        self.labelCpf.setStyleSheet("border: 1px solid black;\n"
"border-radius: 5px;\n"
"background-color: white;\n"
"font-size: 35px;\n"
"color: green;")
        self.labelCpf.setText("")
        self.labelCpf.setAlignment(QtCore.Qt.AlignCenter)
        self.labelCpf.setObjectName("labelCpf")
        self.gridLayout.addWidget(self.labelCpf, 2, 0, 1, 3)
        MainWindow.setCentralWidget(self.centralwidget)

        self.retranslateUi(MainWindow)
        QtCore.QMetaObject.connectSlotsByName(MainWindow)

    def retranslateUi(self, MainWindow):
        _translate = QtCore.QCoreApplication.translate
        MainWindow.setWindowTitle(_translate("MainWindow", "CPF"))
        self.btnGenerator.setText(_translate("MainWindow", "Gerar"))
        self.btnValidate.setText(_translate("MainWindow", "Validar"))
        self.label.setText(_translate("MainWindow", "Gerador de CPF: "))
        self.label_2.setText(_translate("MainWindow", "Validador de CPF: "))
